...
class Interpreter(object):
    ...
    def expr(self):
        ...
        left_value = 0
        while(self.current_token.type != PLUS and self.current_token.type != MINUS):
            if self.current_token.type == INTEGER:
                left_token = self.current_token
                self.eat(INTEGER)
                left_value = (left_value * 10) + left_token.value
            
            if self.current_token.type == SPACE:
                self.space_eat(SPACE)
        ...
        while(self.current_token.type != EOF):
            if self.current_token.type == INTEGER:
                ...
                self.eat(INTEGER)
                right_value = (right_value * 10) + right_token.value
            if self.current_token.type == SPACE:
                self.space_eat(SPACE)
